...
Thre = floor(Thre);
set(handles.text_thre,'string',['选定灰度阈值：',num2str(Thre)]);       
...
D = smooth3(D);
%ui对话框获取体素尺寸mm/pixel 毫米/像素
si = inputdlg({'x方向(毫米/像素)','y方向(毫米/像素)','z方向(毫米/像素)'},'CT扫描参数');
...
if strcmp(sel,'open')                       
    ...
    if n == 1
        ...
        daspect([0.2 0.2 0.18])
        view(-40,24)
        camlight(0,0)                                % create two lights
        camlight(90,90)
        lighting gouraud
        xlabel('X/mm')
        ylabel('Y/mm')
...
% --- Executes on button press in pushbutton_ballmod.
...
% --- Executes on selection change in listbox_ballmod.
...
fid1 = fopen(['' srcDir '\PR全部孔隙.txt'],'wt');
...
% --- Executes on button press in pushbutton_PSDexp.
...
for i = 1:a
    N = PSD{i,1};
    if i == 1
        fid1 = fopen(['' srcDir '\PSD全部孔隙.txt'],'wt');
    else
